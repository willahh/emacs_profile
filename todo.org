...
- [ ] Kill texte : Ne pas ajouter dans le kill ring
      --> L'ajout dans le kill ring ne se fait que depuis M-w si possible
...
- [ ] OSX Unbind C-M-4 : Impossible actuellement de faire du query replace
- [ ] JS2 mode, le refresh du display est lent
...
- [ ] Fichiers .sql mode log edit
- [ ] [multi-cursor] Faire fonctionner le binding sur C-m
- [ ] [vc-diff mode] Voir si il est possible de laAncer des commandes de type ediff
       Depuis  le vc-diff
...
- [ ] [theme] [highlight-symbol] Mettre une couleur moins flashy
