...
#+STARTUP: content
* Utils
- [org-mode] Editer un bloc de code : "C-'"
...
* Bloquant
:VISIBILITY: all
- [ ] [expand-region] Ne pas ajouter d'entrées dans le mark ring
- [ ] Sauvegarder la position du curseur dans le buffer
- [ ] Ajouter des entrées avec Dabrev
- [ ] [C-w] Soit trouver comment utiliser backward delete word ou
  suppression de la sélection courante,
  soit remettre le comportement par default


- [ ] [js2-mode] [company] Add Yas snippnet dans la liste des possibiltées
- [ ] Utiliser Projectile recent previous next comme fonction pour next previous buffer (C-tab / C-S-tab)
- [ ] Hydra for help
...
- [ ] F4 : Faire un hydra sur la touche F4 pour la gestion des window
...
- [ ] [ag] Ouvrir les resultats dans la même fenêtre
  - Semble bon @home
- [ ] [vc-dir] Faire un Hydra pour ce mode sur la touche "."
...
- [X] CONTROL Z : Recenter
- [X] Right alt for HYPER key
- [X] Hyper n p pour monter descendre ligne
- [X] [IDE] Ajouter un bind sur F11 pour afficher le panneau flycheck-list-errors tout en bas
- [X] [diff-hl] Le package est casse sur melpa il ne compile pas bien,
- [X] [vc-dir] Déclencher le vc-dir en mode fullscreen avec la touche
  ESCAPE pour revenir sur le layout précédent comme dans magit
  Un plugin existe pour ça
