...
** Bloquant
- [ ] [swoop] Faire des M-v dans le prompt
...
- [ ] Mouse shift click supprimer la sélection bizarre...
- [ ] Intégrer le M-s pour save plutôt que C-x C-s
- [X] M-W ne ferme plus la frame
- [ ] M-n dans un buffer dired doit faire un nouveau buffer
- [ ] [org-mode] Le toggle des headline doit se faire en deux fois pour que tout fonctionne bien
- [ ] !!! [hydra][helm mini nav] Ne pas activer Hydra moves dans helm
