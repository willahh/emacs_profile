...
* Tasks
:VISIBILITY: all
** TODO Ajouter un bind C-space en mode company pour afficher / masquer la documentation (comportement moderne)
** TODO Ag result buffer : C-o : Open in new window
** TODO Activer le highlight en mode php
** TODO General : Utiliser le package general et jouer une touche sur
  super-spc ou hyper-sp
** TODO Terminer intégration de hunspell @work
** TODO Mini buffer > M-p : Ne pas afficher de message beginning of
  buffer bla bla
** TODO AG mettre en mode emacs et non pas evil normal
** TODO [vc-dir] N et P pour se déplacer en evitant les dossiers
** TODO Help mode evil : add q binding pour quitter
** TODO [projectile find file] Auto centrer la ligne une fois le goto dans le fichier
** TODO [projectile find file] Possibilité de saisir des espaces dans le regexp
** TODO New frame : centrer la frame
** TODO New frame : Mettre un nouveau buffer de type texte dedans
** TODO [vc-dir] Faire un Hydra pour ce mode sur la touch "."
** TODO [svn-log-view] Faire des ediff depuis ce mode
** TODO [swoop] C-w ne fonctionne pas
** TODO [helm] [mini-buffer] Ajouter la possibilité de faire du M-v dans le mini-buffer
** TODO [system] mysql.server start on startup
** TODO [Ibuffer] sort by recent by default
** TODO [diff-hl] Le package est casse sur melpa il ne compile pas bien,
  Comme la fonction custom du mode Dired end start
** TODO [doc-block] La fonction ne fonctionne pas correctement
  ** [php-mode]
  ** [typescript-mode]
  ** [css-mode-mode]

** TODO [org] Organiser les fichiers org
...
** TODO [org-mode] Ne pas faire de traitement special sur la touche tab
** TODO [vc-dir] Ne pas afficher les lignes dossier grises
** TODO [vc-dir] key M-< && M-> : Faire pointer sur le vrai début/fin de ligne
** TODO Previous/Next buffer : Utiliser la liste de ibuffer trier par ordre recent
** TODO web-mode single quote dans commentaire : Ne pas auto fermer la quote lorsque l'on est dans un commentaire
   [2017-10-23 Mon]
...
** TODO Faire fonctionner la custom theme dark pour org mode
   [2017-10-30 Mon]
   [[file:~/.emacs.d/lisp/theme-dark2.el]]

** TODO js2-mode, voir pourquoi la naviguation au sein de fichiers en mode js2, est lente (visible lorsque l'on se déplace sur une ligne de façon répétitive par example)
   [2017-10-30 Mon]
...
* A ranger
** TODO [doc-view] Faire fonctionner doc-view (Lire des fichiers excel, doc, word, etc)
** TODO [Google talk] : Intégrer Google Talk dans Emacs
  https://www.emacswiki.org/emacs/GoogleTalk

** TODO [Helm] Masquer la première ligne
** TODO Back to previous window highlight current line comme lorque l'on fait un M-; en typescript-mode
** TODO [company] Voir pour utiliser autre chose que company pour la complétion des mots car cela peut être lent
...
** TODO Rebind les scroll to other window pour avoir un scroll sur la fenetre a cote mais d'une demie page seulement
...
** TODO Kill texte : Ne pas ajouter dans le kill ring
      --> L'ajout dans le kill ring ne doit se faire que depuis M-w si possible
** TODO M-S-n sur buffer vc-diff ne fonctionne pas 
** TODO [counsel-find-file] [mini-buffer] Ajouter un bind C-k pour supprimer tout le texte
** TODO [dired] Delete dir slow, piste : Tester sans 
** TODO [ffap] Ffap doit fonctionner avec ce genre de lignes :
  - require_once(dirname(__FILE__) . '/file.inc.php');
** TODO [vc-diff mode] Voir si il est possible de lancer des commandes de type ediff
       Depuis  le vc-diff
** TODO Autres tâches à mettre au format todo
...
** TODO [custom] Faire une fonction custom delete backward or region
      C-w doit appeler clean-aindent--bsunindent ou "whole-line-or-region-kill-region"

** TODO [vc-dir] Faire une fonction pour activer un vc-version-diff avec les paramètres older revision : "base", newer revision : "head"

** TODO [util] Revert buffer : keep current line position
** TODO [theme] Mettre plus en avant le buffer actif
** TODO [theme] js color var
** TODO [theme] Ajouter des differenciations sur certains mot clee comme "$", "=", ".", "'", "(", ")", "+", "-", certains mots clés php "var_dump", "PHP_EOL", etc
              Voir package "highlight-chars"
** TODO [theme] Revoir toutes les couleurs pour avoir les mêmes que dans Sublime
      Le profile colorimétrique utilisé par Photoshop n'était pas bon ... ... ...
** TODO [theme] php mode : Plus de couleurs

** TODO [util] Voir si il est possible de faire un (CMD+P + touche @) (file.php@methode_name)
** TODO [vc-dir] Faire une fonction toggle des hunk comme dans magit
** TODO Faire une fonction "Merge lines" pour supprimer les linebreak/whitespace
** TODO Delete file : ne pas re demander le chemin alors que l'on a deja le focus
** TODO [theme] nlinum : highlight current line number
** TODO Ajouter une méthode copy file name pour retourner uniquement le nom du fichier
...
** TODO delete-file
  ...
  - kill buffer
** TODO ag/helm-ag/helm-ag-... : Ignore les regles suivantes :
  - "*.min.js"
  - "*.svn-base"
  - "*.log"
** TODO helm-do-grep-ag -> Ignjorer les fichier "*.svn-base"
...
** TODO ag search result buffer : highlight search words
** TODO Ajouter hook pour recentrer sur la ligne courante après les actions suivantes :
 (recenter)
  - [ ] git-gutter next / prev
  - [x] evil search
  - [ ] highlight mode nav

** TODO Custom theme, se baser sur les couleurs suivantes "images/theme.png"
         http://blog.pabuisson.com/2014/07/vim-vundle-gestion-plugins/

** TODO svn ignore rules to add :
  - .ac-php-conf.json
  - logs
  - .DS_Store
* Mineur
** TODO [dired] delete folder very slow
** TODO Ivy : Possibilité de boucler

** TODO [IMPORTANT] Fonction get file name to clipboard (très utilisé)
** TODO [vc-dir] N'affiche pas les fichiers supprimés localement, mais non envoyé à svn (svn remove)


** TODO [misc] Copier : Ne pas embarquer la couleur
      -> Cibler uniquement certains buffers
        -> mini buffer
        -> fundamental
** TODO nlinum in fringe ? Pouvoir afficher les vc diff plus loin que le contenu
** TODO [vc-dir] Ne pas afficher les lignes dossiers (les lignes qui sont grisées)
** TODO Voir pour utiliser une largeur max lors de la saisie de long texte
      -> Impossible
** TODO web mode : refresh colors after yank
** TODO git-svn
** TODO Fichiers markdown mette toutes les windows en centré
      (Creation de differences inutiles sur la toute dernière ligne de certains fichiers)
** TODO Markdown mode style like in Sublime (background white, text black, column width limit, center inside)
** TODO Dired : remove folder is very slow
** TODO Display current encoding
** TODO Markdown in white theme
** TODO Parcourir partage smb 
** TODO [theme] Color in shell mode 
** TODO [theme] Les couleurs ne sont pas assez décomposé comme dans Sublime (« var » colorA, = colorB, …) 
** TODO Markdown in white theme
** TODO git-svn
** TODO Don't prompt for delete recursivly (code available in prelude)
  - [ ] Show current mode in color
  - [ ] Show current VC system used
  - [ ] Show current encoding
** TODO Remettre un bind pour org mode pour faire des checkbox
** TODO Mini buffer : Ne pas afficher de scrollbar
** TODO Ajouter un bind ALT + arrow up / down sur drags
** TODO [helm] Plugin helm utisable avec la souris
** TODO org mode : > yas snippet new checkbox ne fonctionne plus
  - Retrouver le racourcis de base plutot
** TODO Multi-cursor : KP ENTER -> Meme comportement que ENTER
** TODO [ag] Ne pas lancer la recherche avec les caracteres sur le curseur
** TODO [man] Autofocus sur la fenêtrec
** TODO Associer les fichiers .pdf au mode PDFView
* Corrige
- [X] rebind Sépartions verticales et horizontales sur clavier azerty
- [X] Remettre le binding par default C-q


- [X] ag bufer : hihglight search string
...
- [X] Les splitters verticaux et horizontaux ne sont de nouveaux plus accessibles
- [X] Wndow divider bottom right ne fonctionne pas
